...
Describe "Get-DeploymentResult" {
    ...
    Context "Backwards Compatibility" {
        It "Has aliased 'Get-DeploymentResult' to 'Get-CCMDeploymentResult" {
            $Command = Get-Command -Name Get-DeploymentResult
            $Command | Should -Not -BeNullOrEmpty
            $Command.CommandType | Should -Be Alias
            $Command.ResolvedCommandName | Should -Be "Get-CCMDeploymentResult"
        }
    }
}